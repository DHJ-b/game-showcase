...
ResourceManager::requireTexture(std::string label) {
  ...
  auto labelptr = config.find(label);
  if (labelptr == config.end()) {
    logger->log("required " + label + " but is not found in config", 3);
    return textures["EMPTY"];
  }
  std::shared_ptr<sf::Texture> texture = std::make_shared<sf::Texture>();
  if (!texture->loadFromFile(labelptr->second)) {
    logger->log(
        "required " + label + " but can not open file " + labelptr->second, 3);
    return textures["EMPTY"];
  }
...
ResourceManager::requireFont(std::string label) {
  ...
  fonts[label]->loadFromFile(labelptr->second);
  return fonts[label];
}
